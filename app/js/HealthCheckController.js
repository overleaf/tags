...
module.exports = (HealthCheck = { 
			...
			return request.post({
				...
				return request.get({
					...
					for (tag of Array.from(tags)) {
						var needle;
						logger.log({tag, project_id: project_id.toString()}, "checking tag");
						if ((needle = project_id.toString(), Array.from(tag.project_ids).includes(needle))) {
							hasTag = true;
							break;
						}
					}
					...
					return request.del({
						...
						const otherTags = ((() => {
							const result = [];
							for (tag of Array.from(tags)) { 								if (tag._id !== tag_id) {
									result.push(tag);
								}
							}
							return result;
						})());
						return HealthCheck._removeOldTags(user_id, otherTags, () => callback(err, res, body));
					});
				});
			});
		});
	...
	_removeOldTags(user_id, tags, callback) {
		...
		const oldTags = ((() => {
			const result = [];
			for (tag of Array.from(tags)) { 				if (getAge(tag) > (5*60*1000)) {
					result.push(tag);
				}
			}
			return result;
		...
		const removeTag = function(tag, cb) {
			...
			return request.del({
				...
				json: true
			}, err => cb()); // ignore failures removing old tags
		};
