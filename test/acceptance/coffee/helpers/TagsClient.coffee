...
module.exports =
	getUserTags: (user_id, callback = (error, res, body) ->) ->
		request.get {
			url: "/user/#{user_id}/tag"
			json: true
		}, callback

	createTag: (user_id, name, callback = (error) ->) ->
		request.post {
			url: "/user/#{user_id}/tag"
			json: {name}
		}, callback

	updateTagUserIds: (old_user_id, new_user_id, callback = (error) ->) ->
		request.put {
			url: "/user/#{old_user_id}/tag"
			json: {user_id: new_user_id}
		}, callback

	renameTag: (user_id, tag_id, new_name, callback = (error) ->) ->
		request.post {
	...
	removeProjectFromAllTags: (user_id, project_id, callback = (error) ->) ->
		request.del {
