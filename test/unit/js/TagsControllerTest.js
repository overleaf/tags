...
describe('Tags controller', function() {
  ...
  describe('getUserTags', function() {
    return it('should ask the tags repository for the users tags', function(done) {
      const stubbedTags = [{ name: 'some tag', project_ids: ['12354'] }]
      this.tagsRepository.getUserTags = sinon
        .stub()
        .callsArgWith(1, null, stubbedTags)
      const req = {
        params: {
          user_id,
          project_id
      ...
      return this.controller.getUserTags(req, {
        json: result => {
  ...
  describe('removeProjectFromAllTags', function() {
    return it('should tell the repository to remove that project from all the users tags', function(done) {
      this.tagsRepository.removeProjectFromAllTags = sinon
        .stub()
        .callsArgWith(2)
      const req = {
        params: {
      ...
      return this.controller.removeProjectFromAllTags(req, {
        send: result => {
          this.tagsRepository.removeProjectFromAllTags
